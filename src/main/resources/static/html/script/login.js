        ...
        var login = {
            ...
            phoneSend: function(o) {
                ...
                $.ajax({
                    type: "post", //请求方式
                    dataType: "json",
                    url: path.base + "/user!phoneVerify", //发送请求地址
                    async: false,
                    data: { //发送给数据库的数据
                        phone: $("#phone").val(),
            ...
            checkSecurity: function(vr) {
                ...
                $.ajax({
                    type: "post", //请求方式
                    dataType: "json",
                    url: path.base + "/user!verify", //发送请求地址
                    data: { //发送给数据库的数据
                        verifyCode: vr.val(),
                        byName: byName
                    },
                ...
                if (strName == 'userName') {
                    ...
                    if (!/^[a-zA-Z][a-zA-Z0-9_]{6,15}$/.test(strVal)) {
                        ...
                        $.ajax({
                            type: "post",
                            dataType: "json",
                            url: path.base + "/user!verify1", //发送请求地址
                            data: {
                                verifyStr: strName,
                                verifyVal: strVal
                            },
                ...
                if ($('#userName').val() == null || $('#userName').val() == '') {
                    ...
                    if (subFlag == "1") {
                        $.ajax({
                            ...
                            async: false,
                            cache: false,
                            url: path.base + "/user!register", //发送请求地址
                            data: {
                                "userName": $('#userName').val(),
                            ...
                            success: function(data) {
                                if (data.msg == '1') {
                                	if (!asdfasdf){
                                		 window.location = path.base + "/user!registerFinish?userName=" + data['userName'] + "&time=" + new Date();
                                	} else {
            ...
            validateInvist: function() {
                $.ajax({
                    url: path.base + '/user!checkInvistUser',
                    type: 'GET',
                    dataType: 'json',
                    data: {
                        i_n: $invist.val()
